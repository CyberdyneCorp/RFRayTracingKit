...
## Purpose
Command-line front-ends over the existing importers/simulator/exporters: rftrace-cli (load a scene, run run/runCoverage/runRoute, export by extension-inferred format), rftrace-scene-validator (summary + validation with a non-zero exit on invalid input), and rftrace-result-converter (result-format conversion). Hand-rolled in-tree argument parsing, no new dependency, behind RFTRACE_BUILD_CLI (default ON).
## Requirements
