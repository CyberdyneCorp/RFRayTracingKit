# batched-simulation Specification

## Purpose
Make the simulator drive the ray backend with batched, caller-owned-output queries for its
independent-ray stages (LOS occlusion, the ray-launch wavefront) instead of per-ray calls, so GPU
backends service a whole run in a handful of device dispatches rather than per-ray host↔device round
trips — while producing results bit-for-bit identical to the per-ray path and leaving the CPU
backend unaffected.
## Requirements
