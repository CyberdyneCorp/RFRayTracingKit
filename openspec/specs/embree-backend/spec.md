# embree-backend Specification

## Purpose
An Intel-Embree CPU traversal backend implementing IBackend (SIMD-accelerated BVH), selected via Backend::Embree behind RFTRACE_ENABLE_EMBREE and validated against the reference C++ CPU BVH by a parity suite. Float32 boundary like the GPU backends; the pure-C++ CPU backend remains the always-available default, reference oracle, and fallback.
## Requirements
