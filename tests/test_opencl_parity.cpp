// CPU-vs-OpenCL parity test suite. Built only when RFTRACE_HAVE_OPENCL is
// defined (RFTRACE_ENABLE_OPENCL=ON with an OpenCL device found at configure
// time); every test skips at runtime when no OpenCL device is present, so the
// default CI is unaffected.
//
// The OpenCL BVH traversal kernel works in float32 while the CPU reference (the
// BVH) works in double. Parity therefore compares hit/miss and triangle index
// exactly for well-separated geometry, and t within a tolerance (D4). Where
// float32 rounding legitimately flips a result — a ray grazing a triangle edge,
// sitting on the [tMin,tMax] boundary, or a near-tie between two triangles at
// almost the same depth — the disagreement is classified as "borderline"
// (verified against the exact double-precision geometry) rather than counted as
// a failure. Genuine traversal bugs produce large, non-borderline discrepancies
// and are asserted to be zero. This mirrors the Metal parity suite.
...
ParityStats runScene(const std::vector<Triangle>& tris, unsigned seed,
  ...
  auto gpu = makeBackend(Backend::OpenCL, false);
  cpu->build(tris);
  gpu->build(tris);
...
bool openclReady() { return backendAvailable(Backend::OpenCL); }

}  // namespace

TEST(OpenclParityScenes, EmptyScene) {
  if (!openclReady()) GTEST_SKIP() << "no OpenCL device available";
  auto cpu = makeBackend(Backend::CPU, false);
  auto gpu = makeBackend(Backend::OpenCL, false);
...
TEST(OpenclParityScenes, EmptyBatchReturnsEmpty) {
  if (!openclReady()) GTEST_SKIP() << "no OpenCL device available";
  auto gpu = makeBackend(Backend::OpenCL, false);
  gpu->build(makeWall());
...
TEST(OpenclParityScenes, SingleWall) {
  if (!openclReady()) GTEST_SKIP() << "no OpenCL device available";
  const auto s = runScene(makeWall(), 100, kClosestRays, kOccRays);
  expectParity(s);
}

TEST(OpenclParityScenes, TwoBuildingCanyon) {
  if (!openclReady()) GTEST_SKIP() << "no OpenCL device available";
  const auto s = runScene(makeCanyon(), 200, kClosestRays, kOccRays);
  expectParity(s);
}

TEST(OpenclParityScenes, HundredsOfRandomTriangles) {
  if (!openclReady()) GTEST_SKIP() << "no OpenCL device available";
...
TEST(OpenclParityScenes, ExplicitOccludedClearSegments) {
  if (!openclReady()) GTEST_SKIP() << "no OpenCL device available";
  auto cpu = makeBackend(Backend::CPU, false);
  auto gpu = makeBackend(Backend::OpenCL, false);
...
// The OpenCL dispatch must be deterministic: the same rays twice yield identical
// results (bit-for-bit on the reported fields).
TEST(OpenclParityScenes, DeterministicAcrossRuns) {
  if (!openclReady()) GTEST_SKIP() << "no OpenCL device available";
  auto gpu = makeBackend(Backend::OpenCL, false);
