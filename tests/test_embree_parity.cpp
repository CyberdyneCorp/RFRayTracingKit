// CPU-vs-Embree parity test suite. Built only when RFTRACE_HAVE_EMBREE is
// defined (RFTRACE_ENABLE_EMBREE=ON with Embree 4 found at configure time);
// every test skips at runtime when no Embree device is present, so the default
// CI is unaffected and the file compiles to a no-op in the default build.
//
// The Embree acceleration structure and traversal work in float32 while the CPU
...
// than counted as a failure. Genuine traversal bugs produce large, non-borderline
// discrepancies and are asserted to be zero. This mirrors the CUDA/Metal/OpenCL
// parity suites.
//
// Embree is a CPU ray tracer, so unlike the GPU suites these tests actually run
// on any host where an Embree device can be created; they GTEST_SKIP only if
...
ParityStats runScene(const std::vector<Triangle>& tris, unsigned seed,
  ...
  auto gpu = makeBackend(Backend::Embree, false);
  cpu->build(tris);
  gpu->build(tris);
...
bool embreeReady() { return backendAvailable(Backend::Embree); }

}  // namespace

TEST(EmbreeParityScenes, EmptyScene) {
  if (!embreeReady()) GTEST_SKIP() << "no Embree device available";
  auto cpu = makeBackend(Backend::CPU, false);
  auto gpu = makeBackend(Backend::Embree, false);
...
TEST(EmbreeParityScenes, EmptyBatchReturnsEmpty) {
  if (!embreeReady()) GTEST_SKIP() << "no Embree device available";
  auto gpu = makeBackend(Backend::Embree, false);
  gpu->build(makeWall());
...
TEST(EmbreeParityScenes, SingleWall) {
  if (!embreeReady()) GTEST_SKIP() << "no Embree device available";
  const auto s = runScene(makeWall(), 100, kClosestRays, kOccRays);
  expectParity(s);
}

TEST(EmbreeParityScenes, TwoBuildingCanyon) {
  if (!embreeReady()) GTEST_SKIP() << "no Embree device available";
  const auto s = runScene(makeCanyon(), 200, kClosestRays, kOccRays);
  expectParity(s);
}

TEST(EmbreeParityScenes, HundredsOfRandomTriangles) {
  if (!embreeReady()) GTEST_SKIP() << "no Embree device available";
...
TEST(EmbreeParityScenes, ExplicitOccludedClearSegments) {
  if (!embreeReady()) GTEST_SKIP() << "no Embree device available";
  auto cpu = makeBackend(Backend::CPU, false);
  auto gpu = makeBackend(Backend::Embree, false);
...
// The Embree dispatch must be deterministic: the same rays twice yield identical
// results (bit-for-bit on the reported fields).
TEST(EmbreeParityScenes, DeterministicAcrossRuns) {
  if (!embreeReady()) GTEST_SKIP() << "no Embree device available";
  auto gpu = makeBackend(Backend::Embree, false);
...
TEST(EmbreeParityScenes, CallerOwnedOutputMatchesVector) {
  if (!embreeReady()) GTEST_SKIP() << "no Embree device available";
  auto gpu = makeBackend(Backend::Embree, false);
...
// End-to-end validation: a full LOS coverage run on the Embree backend must
// agree with the CPU backend. With maxReflections=0 the ONLY backend query is
// LOS occlusion (the received power is host-double FSPL), so wherever the float
// Embree traversal and the double CPU BVH agree on occlusion the cell power is
...
// through Simulator::runCoverage on the real Embree backend.
TEST(EmbreeFullSim, LosCoverageAgreesWithCpu) {
  if (!embreeReady()) GTEST_SKIP() << "no Embree device available";
  ...
  auto runOn = [&](Backend b) {
    ...
    s.backend = b;
    s.allowBackendFallback = false;  // force the real Embree backend, no CPU fallback
  ...
  const CoverageResult gpu = runOn(Backend::Embree);
